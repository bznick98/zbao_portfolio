  ...
  return (
    <div ref={containerRef} className="w-full px-6 md:px-12 pt-32 pb-24 min-h-screen bg-[#faf9f6]">
      <div className="max-w-[1800px] mx-auto">
        <h1 className="journal-row text-[12vw] leading-[0.8] font-bold tracking-tighter uppercase mb-24 text-[#111]">Journal</h1>

        <div className="w-full">
            <div className="grid grid-cols-12 border-b border-black pb-4 mb-4 text-sm uppercase tracking-widest opacity-50 journal-row">
            ...
            {ENTRIES.map((entry, i) => (
                <div key={i} className="journal-row group grid grid-cols-12 py-8 border-b border-black/10 items-baseline hover:bg-white transition-colors duration-300 cursor-pointer">
                    <div className="col-span-2 font-mono text-sm opacity-60">{entry.date}</div>
                    <div className="col-span-8 text-3xl md:text-5xl font-serif group-hover:italic transition-all duration-300">{entry.title}</div>
                    <div className="col-span-2 text-right text-xs uppercase tracking-widest border border-black/20 rounded-full py-1 px-3 self-center justify-self-end">{entry.category}</div>
                </div>
            ))}
        </div>
      </div>
